...
21. [Makefile Commands (Quick Reference)](#-makefile-commands-quick-reference)
...
│  9. 📋 SCA scan (OWASP — vulnerable dependencies)   │
...
| File | What it does |
|------|-------------|
| `security-gate.js` | **The security gate.** Reads SCA, Trivy, and Gitleaks reports. Exits with error if Critical/High vulns or secrets found. This is what blocks bad deployments! |
...
- `reports/sca/` — OWASP Dependency-Check results
...
### What is SCA?
**Software Composition Analysis** — checks your dependencies (npm packages) for known vulnerabilities. Tool used: **OWASP Dependency-Check**.
...
| 9 | **SCA - OWASP Dependency-Check** | Scans npm dependencies for known CVEs | No (writes report, gate checks later) |
...
| 13 | **Security Gate** | 🚨 **THE KEY STAGE.** Runs `scripts/security-gate.js` which reads SCA, Trivy, and Gitleaks reports. Fails if ANY Critical/High vuln or secret is found. | **YES — this is the main blocker** |
...
| **SCA** | OWASP Dependency-Check | npm dependencies for known CVEs | `reports/sca/dependency-check-report.json` | Yes (via Security Gate) |
...
|--------|----------|--------|-------------|
| SCA Report | `reports/sca/dependency-check-report.json` | JSON | Detailed dependency vulnerability data |
| SCA Report (viewable) | `reports/sca/dependency-check-report.html` | HTML | Open in browser to see a nice report |
...
**Cause:** Your AWS credentials don't have the required permissions.

**Fix:** Add the permissions listed in the [AWS IAM Permissions](#-aws-iam-permissions-required) section to your IAM user/role.
...
**Fix:** Ensure the `ENABLE_SONARQUBE` parameter is set to `false`. The `when { expression { return params.ENABLE_SONARQUBE } }` block in the Jenkinsfile skips those stages.
...
**A:** No! SonarQube is optional. Keep `ENABLE_SONARQUBE` set to `false` and the SAST/Quality Gate stages will be skipped. The other security scans (OWASP, Trivy, Gitleaks, Syft) will still run.
