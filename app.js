...
// Ready promise that resolves when database initialization is complete
// This allows tests to await app.ready before running
let readyResolve;
...
let activities = [
  ...
  },
  {
    ...
    detail: "Version 1.1.0 deployed successfully",
  },
];
...
app.put("/api/emails/:id", (req, res) => {
  ...
  if (oldFolder === "drafts" && folder === "sent") {
    logActivity(
      "email_sent",
      "user@example.com",
      `Sent draft email to ${email.to}: ${email.subject}`,
    );
  } else if (folder === "drafts") {
    logActivity(
      "draft_updated",
      "user@example.com",
      `Updated draft: ${email.subject}`,
    );
  }
...
app.delete("/api/emails/:id", (req, res) => {
  ...
  if (email.folder === "trash") {
    logActivity(
      "email_deleted_permanent",
      "user@example.com",
      `Permanently deleted email: ${email.subject}`,
    );
    ...
    logActivity(
      "email_trashed",
      "user@example.com",
      `Moved email to trash: ${email.subject}`,
    );
    email.folder = "trash";
