...
// Wait for the app to be ready (database initialization complete) before running tests
beforeAll(async () => {
  await app.ready;
});
...
describe("Mail System API Tests", () => {
  ...
  test("POST /api/emails should create a new email", async () => {
    ...
    const response = await request(app).post("/api/emails").send(newEmail);
  ...
  test("POST /api/emails should block empty emails", async () => {
    const response = await request(app).post("/api/emails").send({}); // Empty email data
